#--------------------------------------------------------------------------------------------------------------------------------------------------------------------
#
#   TensileLoading_Analysis.py
#
#   PURPOSE: Perform analysis on a tensile loading segment
#
...
#
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------
def TensileLoading_Analysis(Raw, Analysis, dir, stage_num, user_opts, plot_opt):
    # Import Modules
    import numpy as np
...
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------
#   TENSILE ANALYSIS
#   Tensile Stage Analysis
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------

    if stage_num == 0:
        # Strain and Stress Rate
        if strain_rate_fit > stress_rate_fit:
            Analysis['Tensile Analysis']['Strain Rate']['Value'] = strain_rate
            Analysis['Tensile Analysis']['Stress Rate']['Value'] = None
        else:
            Analysis['Tensile Analysis']['Strain Rate']['Value'] = None
            Analysis['Tensile Analysis']['Stress Rate']['Value'] = stress_rate
    ...
    if stage_num == 0:
        if user_opts['UserEdit']['Modulus'] == 0:
            Analysis['Tensile Analysis']['Modulus-' + dir]['Value'] = mod
        if user_opts['UserEdit']['Prop'] == 0:
            Analysis['Tensile Analysis']['Proportional Limit-' + dir]['Value'] = prop
            Analysis['Tensile Analysis']['Proportional Limit Strain-' + dir]['Value'] = prop_e
        Analysis['Tensile Analysis']['Poissons Ratio-'+pr_dir]['Value'] = PR

    Analysis['Stages']['Tensile Analysis']['Modulus-' + dir]['Value'].append(mod)
    Analysis['Stages']['Tensile Analysis']['Proportional Limit-' + dir]['Value'].append(prop)
    Analysis['Stages']['Tensile Analysis']['Proportional Limit Strain-' + dir]['Value'].append(prop_e)
    Analysis['Stages']['Tensile Analysis']['Poissons Ratio-'+pr_dir]['Value'].append(PR)
    ...
    if stage_num == 0:
        ...
        Analysis['Tensile Analysis']['Yield-' + dir]['Offset Strain']['Value'] = []
        Analysis['Tensile Analysis']['Yield-' + dir]['Yield Strength']['Value'] = []
        Analysis['Tensile Analysis']['Yield-' + dir]['Strain at Yield Strength']['Value'] = []
        ...
        if "Yield Offset" in list(user_opts.keys()):
            if lin_idx[1] < end_idx-start_idx:
                ...
                for i in range(len(user_opts["Yield Offset"])):
                    ystress, ystrain= CustomYield(strain, stress, Analysis['Tensile Analysis']['Modulus-' + dir]['Value'], user_opts["Yield Offset"][i])
                    
                    if ystress != None:
                        if ystress < max(stress) and ystress > prop:
                            Analysis['Tensile Analysis']['Yield-' + dir]['Offset Strain']['Value'].append(user_opts["Yield Offset"][i])
                            Analysis['Tensile Analysis']['Yield-' + dir]['Yield Strength']['Value'].append(ystress)
                            Analysis['Tensile Analysis']['Yield-' + dir]['Strain at Yield Strength']['Value'].append(ystrain)


    # Delete Previous Calculations
    Analysis['Stages']['Tensile Analysis']['Yield-' + dir]['Offset Strain']['Value'] = []
    Analysis['Stages']['Tensile Analysis']['Yield-' + dir]['Yield Strength']['Value'] = []
    Analysis['Stages']['Tensile Analysis']['Yield-' + dir]['Strain at Yield Strength']['Value'] = []
    ...
    if "Yield Offset" in list(user_opts.keys()):
        if lin_idx[1] < end_idx-start_idx:
            ...
            for i in range(len(user_opts["Yield Offset"])):
                ...
                if ystress != None:
                    if ystress < max(stress) and ystress > prop:
                        Analysis['Stages']['Tensile Analysis']['Yield-' + dir]['Offset Strain']['Value'].append(user_opts["Yield Offset"][i])
                        Analysis['Stages']['Tensile Analysis']['Yield-' + dir]['Yield Strength']['Value'].append(ystress)
                        Analysis['Stages']['Tensile Analysis']['Yield-' + dir]['Strain at Yield Strength']['Value'].append(ystrain)
    ...
    if stage_num == 0:
        if stage_num == len(Analysis['Stages']['Stage Name']['Value'])-1:
            if user_opts['Additional Information'][0] == 'Yes':
                Analysis['Tensile Analysis']['Ultimate Strength-' + dir]['Value'] = max(stress)
                Analysis['Tensile Analysis']['Strain at UTS-' + dir]['Value'] = strain[np.where(stress == max(stress))[0][0]]

    # Save Max Strength Otherwise to Stage Information
    Analysis['Stages']['Tensile Analysis']['Ultimate Strength-' + dir]['Value'].append(max(stress))
    Analysis['Stages']['Tensile Analysis']['Strain at UTS-' + dir]['Value'].append(strain[np.where(stress == max(stress))[0][0]])
        ...
        if stage_num == len(Analysis['Stages']['Stage Name']['Value'])-1:
            if user_opts['Additional Information'][0] == 'Yes':
                ...
                for k in range(1,len(stress)-1):
                    if (stress[k]-stress[k-1])/Analysis['Tensile Analysis']['Ultimate Strength-'+dir]['Value'] < -0.5:
                        fail_idx = k
                        break

                Analysis['Tensile Analysis']['Failure Strength-'+dir]['Value'] = stress[fail_idx]
                Analysis['Tensile Analysis']['Strain at Failure-'+dir]['Value'] = strain[fail_idx]
    ...
    if stage_num == 0:
        if plot_opt == 1:
            ...
            if Analysis['Tensile Analysis']['Modulus-' + dir]['Value'] != None:
                strain_lin_end = (stress[-1]-stress[0])/Analysis['Tensile Analysis']['Modulus-' + dir]['Value']+strain[0]
            ...
            if Analysis['Tensile Analysis']['Proportional Limit-' + dir]['Value'] != None:
                plt.plot(Analysis['Tensile Analysis']['Proportional Limit Strain-' + dir]['Value'],Analysis['Tensile Analysis']['Proportional Limit-' + dir]['Value'],'ro',label='Proportional Limit')

            # -- Plot Yield Behavior
            if Analysis['Tensile Analysis']['Yield-' + dir]['Yield Strength']['Value'] != None:
                for i in range(len(Analysis['Tensile Analysis']['Yield-' + dir]['Yield Strength']['Value'])):
                    plt.plot(Analysis['Tensile Analysis']['Yield-' + dir]['Strain at Yield Strength']['Value'][i], Analysis['Tensile Analysis']['Yield-' + dir]['Yield Strength']['Value'][i],'o',
                             label = str(Analysis['Tensile Analysis']['Yield-' + dir]['Offset Strain']['Value'][i]) + Raw['Raw Data']['Units']['Strain']['Value'] + ' Offset Yield') 

            # -- Plot Ultimate Strength
            if Analysis['Tensile Analysis']['Ultimate Strength-' + dir]['Value'] != None:
                plt.plot(Analysis['Tensile Analysis']['Strain at UTS-' + dir]['Value'], Analysis['Tensile Analysis']['Ultimate Strength-' + dir]['Value'],'mo',label='Ultimate Strength')

            # -- Plot Failure Strength
            if Analysis['Tensile Analysis']['Failure Strength-' + dir]['Value'] != None:
                plt.plot(Analysis['Tensile Analysis']['Strain at Failure-'+dir]['Value'],Analysis['Tensile Analysis']['Failure Strength-'+dir]['Value'],'co',label='Failure Strength')
