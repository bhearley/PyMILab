#--------------------------------------------------------------------------------------------------------------------------------------------------------------------
#
#   CompressiveUnloading_Analysis.py
#
#   PURPOSE: Perform analysis on a compressive unloading segment
#
#   INPUTS:
...
#
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------
def CompressiveUnloading_Analysis(Raw, Analysis, dir, stage_num, user_opts, plot_opt):
    # Import Modules
    import numpy as np
    import scipy.stats
...
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    #   Automatically calculate unloading modulus
    #
    mod_info = scipy.stats.linregress(strain, stress)

    # Save Second Stage Info
    if stage_num == 1:
        Analysis['Compressive Analysis']['Compressive Unloading Modulus-' + dir]['Value'] = mod_info.slope

    # Save All Stage Info
    Analysis['Stages']['Compressive Analysis']['Compressive Unloading Modulus-' + dir]['Value'].append(mod_info.slope)

    #-----------------------------------------------------------------------------------------------------------------------------------------------------------------
    # Reversible and Irreversible Strain
    if abs(stress[-1]) < 1:
        if stage_num == 1:
            Analysis['Compressive Analysis']['Compressive Reversible Strain-' + dir]['Value'] = abs(strain[0]-strain[-1])
            Analysis['Compressive Analysis']['Compressive Irreversible Strain-' + dir]['Value'] = strain[-1]

        Analysis['Stages']['Compressive Analysis']['Compressive Reversible Strain-' + dir]['Value'] = abs(strain[0]-strain[-1])
        Analysis['STtages']['Compressive Analysis']['Compressive Irreversible Strain-' + dir]['Value'] = strain[-1]
...
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------
    if stage_num == 1:
        if plot_opt == 1:
            ...
            # -- Plot the linear fit
            if Analysis['Compressive Analysis']['Compressive Unloading Modulus-' + dir]['Value'] != None:
            ...
            if Analysis['Compressive Analysis']['Compressive Irreversible Strain-' + dir]['Value'] != None:
                plt.plot([0,Analysis['Compressive Analysis']['Compressive Irreversible Strain-' + dir]['Value']],[0,0],'g',label='Irreversible Strain')
            if Analysis['Compressive Analysis']['Compressive Irreversible Strain-' + dir]['Value'] != None:
                plt.plot([Analysis['Compressive Analysis']['Compressive Irreversible Strain-' + dir]['Value'],Analysis['Compressive Analysis']['Compressive Irreversible Strain-' + dir]['Value']+Analysis['Tensile Analysis']['Reversible Strain-' + dir]['Value']],[0,0],'b',label='Reversible Strain')
