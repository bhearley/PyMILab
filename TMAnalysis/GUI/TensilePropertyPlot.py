#--------------------------------------------------------------------------------------------------------------------------------------------------------------------
#
#   TensilePropertyPlot.py
#
#   PURPOSE: Build the Tensile Property Additional Plotting Capability
#    
#   INPUTS:
...
#
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------
def TensilePropertyPlot(self, window, frmt):
    # Import Modules
    ...
    # Get the option
    tens_opt = self.prop_plot_opt.get()

    # Option 1 - Stress vs Strain
    if tens_opt == 'Stress vs Strain':
        ...
        stress_lin = stress[0]+Analysis['Tensile Analysis']['Modulus-'+dir]['Value']*(strain_lin-strain[0])
        ...
        if Analysis['Tensile Analysis']['Proportional Limit-'+dir]['Value'] != None:
            self.plot1.plot(Analysis['Tensile Analysis']['Proportional Limit Strain-' + dir]['Value'],Analysis['Tensile Analysis']['Proportional Limit-'+dir]['Value'],'ro',label='Proportional Limit')

        # -- Plot Custom Yield Offset
        if 'Yield Offset' in list(self.user_opt.keys()):
            for i in range(len(self.user_opt['Yield Offset'])):
                if len(Analysis['Tensile Analysis']['Yield-' + dir]['Strain at Yield Strength']['Value']) > i:
                    strain_val = Analysis['Tensile Analysis']['Yield-' + dir]['Strain at Yield Strength']['Value'][i]
                    stress_val = Analysis['Tensile Analysis']['Yield-'+dir]['Yield Strength']['Value'][i]
                    self.plot1.plot(strain_val,stress_val,'o',label=str(Analysis['Tensile Analysis']['Yield-' + dir]['Offset Strain']['Value'][i]) + Raw['Raw Data']['Units']['Strain']['Value'] + ' Offset Yield')

        # -- Plo Ultimate Strength
        if Analysis['Tensile Analysis']['Ultimate Strength-' + dir]['Value'] != None:
            self.plot1.plot(Analysis['Tensile Analysis']['Strain at UTS-' + dir]['Value'],Analysis['Tensile Analysis']['Ultimate Strength-' + dir]['Value'],'mo',label='Ultimate Strength')

        # -- Plot Failure Strength
        if Analysis['Tensile Analysis']['Failure Strength-' + dir]['Value'] != None:
            self.plot1.plot(Analysis['Tensile Analysis']['Strain at Failure-' + dir]['Value'], Analysis['Tensile Analysis']['Failure Strength-' + dir]['Value'],'co',label='Failure Strength')
    ...
    # Option 2 - Poissons Rato Fit
        ...
        idx_lin = np.where(strain <= Analysis['Tensile Analysis']['Proportional Limit Strain-' + dir]['Value'])[0][-1]

        # -- Create the linear fit
        pr_fit = Analysis['Tensile Analysis']['Poissons Ratio-'+pr_dir]['Value']*(strain[0:idx_lin]-strain[0])-diamstrain[0]
    ...
    elif tens_opt == 'Stress Rate':
        # -- Unpack time and strain for convenience
        time = np.array(Raw['Raw Data']['Time']['Value'][0:Analysis['Stages']['End Index']['Value'][0]])
        stress = np.array(Raw['Raw Data']['Stress-' + dir]['Value'][0:Analysis['Stages']['End Index']['Value'][0]])

        # -- Create the linear fit
        sr_fit = Analysis['Tensile Analysis']['Stress Rate']['Value']*(time-time[0])-stress[0]
